...
| Alpine Linux rootfs base | Mixed (per-package; no single blanket license) | Docker Hub `library/alpine`; exact tag + arm64 digest in `build/sources.lock` | `build/init-rootfs.sh` proves the tag resolves to the approved arm64 digest, scratch-build patches embed the immutable digest-only pull reference in anylinuxfs/init-rootfs, and packaging rejects a floating runtime. The trimmed package set comes from `build/alpine-packages.trimmed.txt`. Each Alpine package carries its own upstream license; consult the [Alpine package license index](https://pkgs.alpinelinux.org/packages) for a given package's specific license if needed.
