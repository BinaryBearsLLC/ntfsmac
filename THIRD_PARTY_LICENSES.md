...
| Alpine Linux rootfs base | Mixed (per-package; no single blanket license) | Docker Hub `library/alpine`; build-time tag + digest in `build/sources.lock` | `build/init-rootfs.sh` verifies the audited build input and installs the trimmed package set from `build/alpine-packages.trimmed.txt`. The shipped upstream-derived runtime still contains an `alpine:latest` initialization default; replacing it with the approved tag/digest is tracked as a P0 roadmap item. Each Alpine package carries its own upstream license; consult the [Alpine package license index](https://pkgs.alpinelinux.org/packages) for a given package's specific license if needed.
