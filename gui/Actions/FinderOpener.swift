...
extension NSWorkspace: WorkspaceOpening {
    public func openPathInFinder(_ path: String) -> Bool {
        ...
        // `open(_:)` may return true for an NFS mount without presenting a Finder window.  Ask
        // Finder to reveal the exact observed mount point first; this API is specific to that UI
        // result and avoids a false-positive success on ntfsmac's NFS-backed volumes.
        if NSWorkspace.shared.selectFile(nil, inFileViewerRootedAtPath: path) {
            NSLog("ntfsmac: Finder reveal succeeded")
            return true
        }

        // `/usr/bin/open` is the reliable fallback for the same path on NFS-backed volumes.
        let process = Process()
        ...
        do {
            ...
            if process.terminationStatus == 0 {
                NSLog("ntfsmac: /usr/bin/open fallback succeeded")
                return true
            }
        } catch {
            NSLog("ntfsmac: /usr/bin/open fallback failed with error: \(error)")
        }

        // Retain the generic Launch Services route for unusual Finder configurations.
        if NSWorkspace.shared.open(url) {
            NSLog("ntfsmac: NSWorkspace open fallback accepted the URL")
            return true
        }

        NSLog("ntfsmac: all Finder open strategies failed")
        return false
    }
}
