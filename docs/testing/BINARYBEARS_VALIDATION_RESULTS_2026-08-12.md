...
| BB-P0-06 | IN PROGRESS | On the current installed package, MobileData was mounted RW with one measured/enforced security session and opened in Finder. Codex then selected the eject control for the synthetic `disk6s1.local` network share, not the physical USB device. At the first post-disconnect observation the host NFS table was empty, diagnostics were healthy with zero mounts/sessions and the bridge down, and the running GUI showed two detected devices with no green mount state. The package then repeated the same behavior after the operator ran authenticated `/sbin/umount /Volumes/MobileData`: without Refresh, GUI and diagnostics again converged to two detected devices, zero NFS mounts/sessions, and bridge down. The final root inspection for security state files and PF child anchors remains. |
