...
| BB-P3-04 | FAIL | A safe read-only working-directory hold on MobileData deterministically produced the intended partial result: MobileData `Still mounted`, USB_8GB `Unmounted`, both rows remained independently actionable, dismissing the report changed presentation only, and releasing the hold allowed MobileData's normal Unmount to recover cleanly. The installed CLI nevertheless accepted anylinuxfs's zero exit before proving the host NFS mount disappeared, tore down MobileData's security state, and briefly left one accessible NFS mount with zero security sessions; diagnostics correctly became unhealthy while the GUI security summary reconciled to `NO_ACTIVE_MOUNTS`. Source now requires authoritative host-mount disappearance before per-session teardown and retains protection when the mount remains or the table is unreadable; the focused shell suite passes `15/15`, the complete shell suite `292/292`, and Swift `247/247`. A rebuilt packaged repeat remains required. |
