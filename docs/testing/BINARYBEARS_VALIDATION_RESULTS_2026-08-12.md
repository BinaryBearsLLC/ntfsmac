...
| BB-P1-00 | IN PROGRESS | The normal row remains Open/Unmount and exposes only Verified Copy in `…`; the destination panel began on the exact MobileData mount and the earlier Cancel created no file/status. During replacement-candidate automation, the native source panel visually selected `gui-source.bin` but dispatched its parent `fixtures` directory, so that run is invalid rather than a product pass/fail. The copy stayed in one hidden partial with no final name and its isolated process group was stopped; mount/security remained healthy. Computer Use then lost only the app's accessibility bridge while Finder remained readable and a process sample showed the GUI main thread idle. The current-candidate retry did not reach the picker: immediately after the completed VPN transition cell, a new GUI mount remained in pre-mount vmnet startup for about 150 seconds with no NFS mount or security session. The GUI truthfully retained `Mounting…` and never went green. The operator terminated only the verified mount process groups; the GUI returned to two detected drives, all backend processes exited, diagnostics showed zero mounts/sessions and bridge down, and the VPN default remained active. This is a mount-lifecycle finding, not a Verified Copy failure. Human source selection plus success/refusal/in-app Cancel remain after a rebuilt watchdog candidate is installed. |
...
gates pass shell `294/294` and Swift `251/251`. Live expiry and retry remain pending on a rebuilt
installed package; the observed current-package recovery is recorded as measured rather than
retroactively credited to the source fix.
