...
| BB-03 | FAIL | Mounted/Settings/Diagnostics light UI, Back, overflow-only copy action and Launch at login enable/disable passed on the prior installed state. On 2026-08-14, the mounted/security popover also passed a temporary Dark appearance check with the host's original Automatic setting restored afterwards; the explicit NTFS3 preflight and failed-closed symlink copy provided truthful warning/error presentation without a published final destination. Keyboard-only validation temporarily enabled Full Keyboard Access and restored its original off state afterwards. Refresh, both driver menus, USB_8GB Mount, and Settings accepted focus, but traversal skipped the first drive's Mount control plus Diagnose and Quit. The later deliberately blank BB-01 install also showed `Launch at login is unavailable for this app bundle` despite installation under `/Applications`. Both findings are reproducible accessibility/lifecycle failures and require a focused fix and packaged retest. |
