...
| BB-F01 | FAIL | On 2026-08-14 USB_8GB was mounted with default `ntfs-3g`; diagnostics proved one enforced private session, and the live gate proved vmnet/private/`soft` NFS with no loopback listener. With no copy/verify process or Finder use, the operator physically removed only that disposable device. `diskutil list external` then showed only MobileData, but after more than 36 seconds the host NFS mount, anylinuxfs VM, vmnet helper, security session, and green GUI `Mounted read/write` row all remained. The `soft` contract avoided a system hang, but bounded transition away from green and automatic teardown failed. The corrected packaged repeat on 2026-08-16 mounted USB_8GB with explicit NTFS3 and MobileData with default `ntfs-3g` as two green rows, NFS mounts, VM sessions, and enforced security sessions. With no copy or test I/O active, physically removing only USB_8GB immediately removed it from authoritative physical-disk enumeration, but for more than 30 seconds its NFS mount, VM, security session, and green GUI row again remained while diagnostics incorrectly stayed healthy at two mounts/sessions. Authorized Unmount on only the stale USB row immediately removed its NFS/VM/session and GUI row while preserving MobileData and its enforced session. Normal MobileData Unmount then reached zero NFS mounts/sessions with the bridge down. The operator-authenticated final check found no security state file or PF child anchor. Recovery and teardown are selective and clean, but still manual; the focused physical-presence reconciliation fix did not activate in the installed app and this row remains `FAIL`. |
