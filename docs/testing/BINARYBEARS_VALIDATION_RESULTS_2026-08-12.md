...
| BB-P0-08 | IN PROGRESS | With no active mount, the operator successfully restarted the exact `system/com.khr898.ntfsmac.helper` job. Read-only launchd/process inspection found one running job record, one helper process (`pid 34218`), and the expected single plist/binary pair; diagnostics remained healthy at zero state. The required first GUI mount/unmount after restart remains. |
