...
- The second physical NTFS fixture then mounted through the installed corrected app with paired
  ...
  bridge down and zero sessions. At that point only the final operator-authenticated state/PF check
  remained for BB-01.
- The final operator-authenticated check returned `BB-01-RW-ROOT: PASS` with no security state file
  or PF child anchor. BB-01 is fully `PASS` on the installed corrected candidate.
...
| BB-01 | PASS | Installed corrected candidate `e9a549…fbdc5` passed explicit helper consent, bounded preparation, Full Disk Access gating, first-click mount startup, helper/CLI/version checks, guest/host truth reconciliation, and clean teardown. A rw-NFS/ro-guest fixture appeared yellow `Mounted read-only` with recovery guidance and no write. A known-clean fixture then mounted read/write; its fresh 4 MiB payload matched byte-for-byte and by SHA-256 after flush/reread, was removed, and GUI Unmount returned host/guest mounts, bridge, and sessions to zero. Both authenticated teardown checks found no security state file or PF child anchor. |
...
| BB-P1-06 | PASS | The 2026-08-14 same-device comparison used USB_8GB first with default `ntfs-3g`, then with explicit NTFS3 after the packaged preflight. Diagnostics proved the requested driver with no fallback and one enforced private session in both cases. A 256 MiB file matched SHA-256 `54ca03f1af5eeb02ca7e562f93a33b2bb77ceaf62f035d51e40142dd330e12b5`; 505 regular entries plus create, copy, overwrite, append, rename, move, delete, Unicode, safe unmount/remount, and post-remount verification passed under both drivers. The full 506-entry tree did not pass the portability gate: NTFS3 exposed an `ntfs-3g` symlink as a 46-byte regular `IntxLNK` file, while a fresh NTFS3 Verified Copy of that symlink failed with `Invalid argument`/`Operation timed out`. The corrected packaged repeat on 2026-08-16 mounted the same disposable drive with explicit NTFS3, with diagnostics proving one enforced private session, the requested driver, and no fallback. Selecting a local directory containing a nested symbolic link produced the specific NTFS3 warning before destination publication; the requested destination stayed absent and no new partial appeared. The no-overwrite sub-gate still failed: entering the name of an existing destination produced AppKit's native destructive `Replace` offer. The operator selected Cancel; an independent tree snapshot remained identical and no copy started. That artifact therefore remained `FAIL` solely on the misleading native Replace path. The rebuilt 2026-08-16 candidate then intercepted the existing name inside the native save-panel delegate: AppKit showed no Replace dialog, the same panel stayed open with direct no-overwrite guidance, the original destination size and SHA-256 remained unchanged, and no copy process or partial appeared. Both scoped fixtures were removed. This closes the failed presentation/data-safety sub-gate and converts the row to `PASS`; the earlier cross-driver `IntxLNK` portability limitation remains documented rather than presented as native Windows-symlink support. |
| BB-P1-07 | PASS | The qualification preserved the earlier Windows clean-state operation matrix, 256 MiB hash match, cross-platform reread, clean dirty queries, and read-only CHKDSK evidence. Hardware then proved the original single NTFS3 probe missed a Windows-confirmed dirty flag. Runtime v3 added dual read-only eligibility checks and the corrected installed package refused the same dirty device with concise fixed guidance, `unsafe_windows_state`, no raw backend transcript, no fallback, no mount/session residue, and an authenticated clean state/PF audit. Windows repaired that same device with `chkdsk /F /X`; the clean Mac repeat mounted explicit NTFS3 over private `soft` NFS, copied/flushed/reread 268435456 bytes, and matched SHA-256 `48c0a5b3b2ead4b640bceb4978be331159ec6ca0322ddc72372f850e27a7c207`. Packaged unmount returned `SESSION_REMOVED`, all mount/backend/security state reached zero, and the authenticated cleanup passed. Windows finally reread the exact relative payload with the same size/hash, reported the volume not dirty, and completed read-only CHKDSK with no problem, required action, or bad sector. Fast Startup and a genuinely hibernated removable-volume fixture remain separately resource-blocked subcells and are not credited as passes. |
...
| BB-F01 | PASS | Earlier 2026-08-14 and 2026-08-16 packages left a physically absent device falsely green with its NFS/VM/security state alive until manual selective recovery; those failures remain retained evidence. The final installed `d513c3f` candidate mounted MobileData with default `ntfs-3g` and USB_8GB with explicit NTFS3 as two independently verified private `soft` NFS sessions. After only USB_8GB was physically removed, enumeration lost it immediately; its NFS mount, VM, session, and GUI row remained through 10 seconds and disappeared automatically by the 12-second observation. MobileData remained read/write and enforced with healthy one-mount/one-session diagnostics, and the popover removed the absent row without Refresh. Normal GUI Unmount of MobileData completed within the first one-second poll and reached zero mounts/backend sessions, bridge down, inactive transport, and `NO_ACTIVE_MOUNTS`. The operator-authenticated final audit found no security state files or PF child anchors. This closes bounded selective hot-unplug reconciliation on the installed package. |
...
BB-03, BB-01, and BB-P1-06 now pass their installed corrected candidates. The live ledger therefore
contains 26 `PASS`, two measured `FAIL`, and two resource `BLOCKED` rows. Remaining failures are
BB-P1-07 and BB-F01; the blocked rows remain BB-P1-05 controlled TV playback and the
BB-P1-08 extended resource matrix.

The 2026-08-16 BB-P1-06 packaged repeat closed the NTFS3 nested-symlink preflight sub-gate without
creating a destination or a new recoverable partial. It also reproduced the native AppKit
`Replace` offer for an existing destination. Cancel left the existing tree unchanged, so the
safety invariant still holds. The final rebuilt repeat prevented that offer, retained the panel
with direct no-overwrite guidance, and preserved destination bytes with no process or partial;
BB-P1-06 is now `PASS`.
...
The two-drive BB-F01 repeat then reproduced the original stale-green defect on the corrected
package. Physical enumeration lost only USB_8GB immediately, but its NTFS3 NFS mount, VM, security
session, and green GUI row survived for more than 30 seconds while MobileData remained present.
Manual Unmount of only the stale row performed exact selective teardown and preserved MobileData;
the final normal unmount reached zero ntfsmac mounts/sessions with the bridge down. The correction
...
The complete pre-recovery automated gates pass Bats `300/300` and Swift `271/271`; the real vendored rebuild
also passes its Rust tests, architecture, entitlement, static-link, rootfs-manifest, and runtime-
contract checks. The later packaged BB-P1-06 repeat passes, so the public ledger is now 26 `PASS`,
2 `FAIL`, and 2 `BLOCKED`; BB-P1-07 and BB-F01 still require rebuilt-DMG repeats. No personal
...
The explicit trusted-binary reconciliation subsequently returned
`STALE_SESSIONS_REMOVED`. Its authenticated audit passed with zero state files, zero PF child
anchors, and a zero-session public summary. The machine is clean again. The two new source fixes
still require a rebuilt package plus one direct CLI mount/unmount regression; this recovery result
does not substitute for that packaged evidence.

With the live state clean, the complete source suite passed Bats `302/302`; the two diagnostic
tests that had correctly failed against the orphan returned green without test-specific changes.
The complete Swift suite also passed `271/271`. Source gates are closed for a rebuild. Packaged
...
Windows then reread the exact relative path
`BB-P1-07-clean-ntfs3-20260817-133935/mac-ntfs3-256MiB.bin` from the same directly attached USB
device. Size was 268435456 bytes and SHA-256 matched
`48c0a5b3b2ead4b640bceb4978be331159ec6ca0322ddc72372f850e27a7c207`. The dirty query reported the
volume clean; read-only CHKDSK exited 0, found no filesystem problem, required no action, and
reported zero bad sectors. Record `BB-P1-07-WINDOWS-REREAD: PASS` and convert BB-P1-07 itself to
`PASS`.

## BB-P1-07 closure checkpoint — 2026-08-17

The current live ledger contains 27 `PASS`, one measured `FAIL`, and two resource `BLOCKED` rows.
BB-F01 selective two-drive hot-unplug reconciliation is the sole measured failure remaining. The
...
The `d513c3f` package was installed and the previously frozen GUI mount completed from `Mounting...`
to the authoritative read/write row in approximately seven seconds. MobileData used default
`ntfs-3g`; USB_8GB used explicit NTFS3. Host NFS truth, runtime state, diagnostics, and the GUI
agreed on two mounts and two enforced security sessions before the no-I/O removal.

After only USB_8GB was physically disconnected, the monitor detected physical absence and retained
both sessions through +10 seconds. At +12 seconds the absent device's NFS mount and security session
were gone; every observation through +30 seconds retained exactly one healthy MobileData mount and
session. The GUI independently removed USB_8GB without Refresh and kept MobileData green with the
three truthful security rows. No process scoped to the absent device remained. Normal GUI Unmount
of MobileData completed within the first one-second poll. Diagnostics then reported zero mounts and
sessions, bridge down, no network helper, inactive transport, and `NO_ACTIVE_MOUNTS`. The public
status file remained `root:daemon 0644` with the same zero-session summary. The final
operator-authenticated audit returned `BB-F01-ROOT: PASS` with no security state files or PF child
anchors.

BB-F01 is `PASS`. The current ledger is 28 `PASS`, zero `FAIL`, and two resource `BLOCKED` rows:
