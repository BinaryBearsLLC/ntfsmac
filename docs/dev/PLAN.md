...
| L10 | **Repository identities are explicit.** BinaryBears `main` mirrors `khr898/ntfsmac`; BinaryBears-only development lives on `dev`. The original upstream and its existing tap/distribution remain `khr898/ntfsmac`. No `YOURUSERNAME` literals and no silent substitution between the two. |
